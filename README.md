...
闭路建立后，“物流实测参数”面板会自动增加所需的回流数据行：普通产品回流填写该回流物流的干质量和品位；产品合并输出回流填写两条合并前回流支路的数据，合并后的总回流由守恒关系自动计算。填写全部终端产品和回流支路后即可计算，程序会反算内部物流以及外部新鲜入料。
